...
void green_and_red2( int repeats )
{
	...
	for ( int n = 0; n < repeats; n++ )
	{
		for ( int i = 0; i < steps; i++ )
		{
			leds[  0 ]	= leds[  3 ]	= leds[ 6 ]	= (double)i / (double)steps;
			wait( delay );
		}
		
		for ( int i = 0; i < steps; i++ )
		{
			leds[  0 ]	= leds[  3 ]	= leds[ 6 ]	= (double)(steps - i) / (double)steps;
			wait( delay );
		}
	
		for ( int i = 0; i < steps; i++ )
		{
			leds[  1 ]	= leds[  4 ]	= leds[ 7 ]	= (double)i / (double)steps;
			wait( delay );
		}
		for ( int i = 0; i < steps; i++ )
		{
			leds[  1 ]	= leds[  4 ]	= leds[ 7 ]	= (double)(steps - i) / (double)steps;
			wait( delay );
		}
	}
}
